...
exports.login=async(req,res)=>{
    try{
        ...
        else{db.query("select * from users where email=?",[email],async(error,result)=>{
            console.log(result);
            if(result.length<=0){
                ...
                if(!(await bcrypt.compare(password,result[0].PASS))){
                    ...
                    res.cookie("Praba",token,cookieOptions);
...
exports.register=(req,res)=>{
   ...
   db.query("select email from users where email = ?",[email],async(error,result)=>{
    ...
    db.query("insert into users set?",{name:name,email,pass:hashedPassword},
    (error,result)=>{
        if(error){
            console.log(error);
        }else{
